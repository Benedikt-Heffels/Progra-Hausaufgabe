public class Tagesgeld {
    ...
    //a)
    //Die Methode ist nicht static, da sie zur Verzinsung auf das aktuelle Tagesgeld-Objekt zugreifen muss
    //Die Methode ist Public, damit man von einer Instanz der Klasse die optimale Verzinsung abrufen kann
    ...
    public static int verkuerzeUmKuerzesteLaufzeit(boolean verkuerze, Tagesgeld... ts){
        if(ts.length == 1){
            return ts[0].getAngebotsmonate();
        }
        
    }

    private static int kuerzesteLaufzeitHelper(boolean verkuerze, Tagesgeld[] ts, int i){
        if(i >= ts.length - 1) {
            return ts[i].getAngebotsmonate();
        }
        int min = Math.min(ts[i].getAngebotsmonate(), kuerzesteLaufzeitHelper(verkuerze, ts, i+1));
        if(verkuerze) {
